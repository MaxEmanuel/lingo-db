...
namespace TypeFunctions
{
     ...
     * database tries to cast the given value to the specified ```mlir::Type```. The main difference to the cast function in the Parser is, that 
     * ```mlir::db::Nullable``` will be treated differently. If the ```toCast``` parameter is explicitly a ```null``` value, then it tries also
     * the cast (needed for some array functions, otherwise they would not working correctly).
     * @param builder           A reference to a ```mlir::OpBuilder```. This object allows to create ```mlir``` 
     *                          operations as well as types and values
     ...
     * @param isNullable        If the source type can be nullable.
     * @return                  An updated version of the given ```mlir::Value``` wrapped inside a ```mlir::db::CastOp```.
     */
    mlir::Value addCastOp(mlir::OpBuilder& builder, const mlir::Value& toCast, mlir::Type type, bool isNullable);
    ...
    /**
     * This function cast a non ```mlir::db::ArrayType``` to this type using some asssumptions (dimension = 1) according its specification. If
     * the given ```type``` is only a ```mlir::db::NullableType``` then it assumes that all elements are of type string.
     * @param builder           A reference to a ```mlir::OpBuilder```. This object allows to create ```mlir``` 
     *                          operations as well as types and values
     * @param value             The ```mlir::Value``` which should be casted
     * @param type              The target type of that cast
     ...
     */
    ArrayData castToArrayIfNecessary(mlir::OpBuilder& builder, const mlir::Value& value, const mlir::Type& type);
}